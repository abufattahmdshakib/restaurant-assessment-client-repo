...
export default function CategoryFilter({
  ...
  return (
    <div className="sm:my-12 my-8 relative flex flex-wrap justify-between px-2 items-center gap-4">
      <div className="flex gap-2 sm:gap-8 flex-wrap">
        {categories.map((cat, idx) => (
          <button
            ...
            className={`px-3 py-1 text-xs sm:px-4 sm:py-2 sm:text-[18px] rounded-full border ${selectedCategory === cat
              ? "bg-black text-white"
      ...
      <div className="flex gap-2 sm:gap-8 flex-wrap relative">
        {/* Add Food */}
        <div className="relative" ref={foodRef}>
          <button
            onClick={() => toggleDropdown("food")}
            className="px-3 py-1 text-xs sm:px-4 sm:py-2 sm:text-[18px] rounded-full bg-black text-white"
          >
            Add Food
          </button>
