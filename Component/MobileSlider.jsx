...
import { items } from "./sliderData"; // shared data
...
export default function MobileSlider() {
    ...
    return (
        <motion.div
            className="relative sm:hidden w-full min-h-[95vh] overflow-hidden px-6 py-8 text-white"
        ...
        >
            ...
            {/* MOBILE SEARCH */}
            <div className="relative z-40 mb-5 flex items-center bg-white rounded-xl px-4 py-1">
            ...
            {/* Heading */}
            <h2 className="relative text-center z-40 text-[34px] font-[400] mb-2">
                BREAKFAST
            </h2>

            {/* Paragraph + Inline Expand */}
            <p className="relative z-40 text-white/85 text-[15px] leading-relaxed mb-4">
            ...
            <div className="relative flex justify-center items-center mb-6">
                ...
                <button
                    onClick={prevSlide}
                    className="absolute left-0 z-30 bg-white/20 backdrop-blur-sm p-2 rounded-full"
                >
                    <ChevronLeft size={26} className="text-white" />
                </button>

                {/* Large Image with smooth slide */}
                <AnimatePresence initial={false} custom={direction}>
                    {/* Large Image */}
                    <motion.div
                        key={active.id}
                        initial={{ x: direction === 1 ? 80 : -80, opacity: 0, scale: 0.8 }}
                        animate={{ x: 0, opacity: 1, scale: 1 }}
                        transition={{ duration: 0.8 }}
                        className="w-[200px] h-[200px] rounded-full overflow-hidden relative z-20"
                    >
                        <img
                            src={active.large}
                ...
                <button
                    onClick={nextSlide}
                    className="absolute right-0 z-30 bg-white/20 backdrop-blur-sm p-2 rounded-full"
                >
            ...
            <div className="relative z-40 flex gap-4 justify-center">
                {items.map((item, i) => (
